...
public class FramePacer {
    ...
    public enum FrameStatus {
        ACCEPTED,               //!< The frame is approved for rendering.
        SKIPPED_SPURIOUS,       //!< Skipped to maintain target frame rate cadence (e.g. 30 FPS on 60Hz display).
        SKIPPED_STALE           //!< Skipped to prevent out-of-order presentation (monotonic guard).
    }
    ...
    public FrameStatus setupFrame(long frameTimeNanos, long vsyncPeriodNanos) {
        return FrameStatus.values()[nSetupFrame(getNativeObject(), frameTimeNanos, vsyncPeriodNanos, null, 0)];
    }
    ...
    public FrameStatus setupFrame(@NonNull Choreographer.FrameData frameData, long vsyncPeriodNanos) {
        ...
        return FrameStatus.values()[nSetupFrame(getNativeObject(), frameData.getFrameTimeNanos(), vsyncPeriodNanos, mCachedTimelines, timelines.length)];
    }
